...
static DBusHandlerResult shtc1_filter_func(DBusConnection *c, DBusMessage *m, void *data)
{
    ...
    char *p_temp_high_threshold = NULL;
    ...
    if (0 == strcmp(iface_path, SHTC1_IFACE_PATH))
    {
        ...
        {
            get_single_arg_from_message(m, (void*)&p_temp_high_threshold);
            dbg(IOT_DEBUG, "temp_high_threshold = %s", p_temp_high_threshold);
            g_temp_high_threshold = atof(p_temp_high_threshold);
        }
    }
    return DBUS_HANDLER_RESULT_HANDLED;
...
void *shtc1_process(void* args)
{
    ...
    while (loop)
    {
        ...
        if (temp1 > g_temp_high_threshold)
        {
            ...
            send_signal(conn, &emit_path_set, DBUS_TYPE_STRING, &p_temp1_buf);
        }
        sleep(5);
    }
}
